...
type AttributeWithActionsType = attributeType & {
  actions?: string
}
...
// Vars
const AttributeData: attributeType[] = [
  {
    id: 1,
    attributeName: 'Smart Phone',
  ...
  {
    id: 2,
    attributeName: 'Clothing, Shoes, and jewellery',
  ...
  {
    id: 3,
    attributeName: 'Home and Kitchen',
  ...
  {
    id: 4,
    attributeName: 'Beauty and Personal Care',
  ...
  {
    id: 9,
    attributeName: 'Computer Accessories',
  ...
  {
    id: 11,
    attributeName: 'Smart Home Devices',
...
// Column Definitions
const columnHelper = createColumnHelper<AttributeWithActionsType>()
...
const ProductAttributeTable = () => {
  ...
  const columns = useMemo<ColumnDef<AttributeWithActionsType, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      columnHelper.accessor('totalEarning', {
        ...
        cell: ({ row }) => <Typography>{row.original.totalEarning.toLocaleString('en-IN', { style: 'currency', currency: 'USD' })}</Typography>
      }),
            ...
            <OptionMenu
              ...
              options={[
                { text: 'Download', icon: 'tabler-download' },
                {
                  text: 'Delete',
                  icon: 'tabler-trash',
                  menuItemProps: { onClick: () => setData(data.filter(Attribute => Attribute.id !== row.original.id)) }
                },
    ...
    <>
      <Card>
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          <DebouncedInput value={globalFilter ?? ''} onChange={value => setGlobalFilter(String(value))} placeholder='Search' className='max-sm:is-full' />
          <div className='flex max-sm:flex-col items-start sm:items-center gap-4 max-sm:is-full'>
            ...
            <Button variant='contained' className='max-sm:is-full' onClick={() => setAddAttributeOpen(!addAttributeOpen)} startIcon={<i className='tabler-plus' />}>
              Add Attribute
            </Button>
          </div>
        </div>
      ...
      <AddAttributeDrawer open={addAttributeOpen} attributeData={data} setData={setData} handleClose={() => setAddAttributeOpen(!addAttributeOpen)} />
    </>
  )
}
