...
import { updateProduct } from '@/libs/api/category.api'
import { ProductForm, Product } from '@/types/app/category'
import { cleanObject } from '@/utils/formatters'
import { handleApiError } from '@/utils/handleApiError'
import ParentProductSelect from './ParentProductSelect'
import ProductThumbnailImage from './ProductThumbnailImage'
import RichTextEditor from '@/components/RichTextEditor/RichTextEditor'
import { errorProductMessage } from '@/messages/auth/categoryMessages.'
...
const UpdateProductModal = ({ children, initialData }: UpdateProductModalProps) => {
    ...
    async (formData: ProductForm) => {
      ...
      try {
        if (initialData?.id !== undefined) {
          ...
          const { status } = await updateProduct(String(initialData.id), changedData)

          const errorMessage = handleApiError(status, errorProductMessage)
          ...
          <Grid container spacing={6}>
            <Grid size={{ xs: 12, md: 6 }}>
              <Grid container spacing={6}>
                ...
                />
                <ParentProductSelect control={control} errors={errors} isLoading={isLoading} />
                <ProductThumbnailImage control={control} errors={errors} setValue={setValue} isLoading={isLoading} category={initialData} />
              </Grid>
            </Grid>
