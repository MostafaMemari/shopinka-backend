...
export default async function Home() {
  const discountProducts = (await getProducts({ take: 14, hasDiscount: true })).items;
  const newestProducts = (await getProducts({ take: 14, sortBy: 'newest' })).items;
  ...
  return (
    <>
      ...
      <main className="grow bg-background pb-14 pt-22 lg:pt-36">
        ...
        <CarouselProduct title="جدیدترین محصولات" products={newestProducts} viewAllLink="/shop?hasDiscount=true" />
      </main>
    </>
  );
}
