  ...
  return (
    <div className="border-gradient group relative rounded-base border p-px before:absolute before:-inset-px before:h-[calc(100%+2px)] before:w-[calc(100%+2px)] before:rounded-base">
      <div className="relative rounded-xl bg-muted p-2 shadow-base md:p-5">
        ...
        <div className="mb-2">
          <Link href={`/product/${slug}`} className="line-clamp-2 h-10 text-sm md:h-12 md:text-base">
            {name}
          </Link>
        </div>
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-background text-red-500 transition-all duration-200 hover:bg-warning hover:text-gray-100 cursor-pointer"
            onClick={onClick}
          >
