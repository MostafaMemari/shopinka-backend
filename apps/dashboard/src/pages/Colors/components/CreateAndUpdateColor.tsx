...
const CreateAndUpdateColor: React.FC<ColorFormProps> = ({ onSuccess, color, onClose }) => {
    ...
    try {
      if (!color) {
        ...
        if (response.status === 201) {
          Toast("رنگ جدید با موفقیت انجام شد", "success");
          handleModalClose();
          onSuccess();
        } else {
        ...
        const response = await updateColorService(Number(color.id), data);

        if (response.status === 200) {
          Toast("بروزرسانی رنگ با موفقیت انجام شد", "success");
          handleModalClose();
          onSuccess();
        } else {
          Toast("خطایی رخ داد، لطفاً دوباره تلاش کنید", "error");
      ...
      Toast("خطایی در ثبت رنگ رخ داد", "error");
    }
  };
  ...
  return (
    <Dialog open={isModalOpen} onClose={handleModalClose} initialFocus={sendButtonRef}>
      <Dialog.Panel>
        <Dialog.Title>
          <h2 className="ml-auto text-start font-medium">ثبت رنگ</h2>
        </Dialog.Title>
